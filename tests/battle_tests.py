...
import library.abilities as abilities
...
test_dummy.health = test_chicken.max_health
test_dummy.level = 5
test_dummy.type = "normal"
test_dummy.defense = 30
...
class BattleTests(unittest.TestCase):
    def test_calculate_damage_1(self):
        ...
        expected_damage = int((((2 * test_chicken.level / 5 + 2) * test_ability.power *
                                (test_chicken.strength / test_dummy.defense)) / 50) + 2)
        actual_damage = battle.calculate_damage(test_chicken, test_dummy, test_ability, is_critical=False)
